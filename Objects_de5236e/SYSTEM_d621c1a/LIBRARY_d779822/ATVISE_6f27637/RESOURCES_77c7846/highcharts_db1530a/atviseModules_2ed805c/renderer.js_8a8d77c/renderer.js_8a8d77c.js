...
Renderer.prototype.runContinuousRendering = function () {
	...
	function subscribeCallback(dataSourceErrors, data) {
		...
		for (var dataKey in data) {
            ...
            /** store points while history load in progress **/
            if(series && self._rendererLoadHistoryInProgress[series]) {
                if(!self.pointStack.hasOwnProperty(series)) {
                    self.pointStack[series] = [];
                }

                for (var i = 0; i < data[dataKey].length; i++) {
                    var p = self.dataHandler.getPoint(data[dataKey][i], series.options.type, dataKey);
                    if (p) {
                        self.pointStack[series].push(p);
                    }
                }
            }
			...
			if (series) {
                ...
                /** add stored points **/
                if(self.pointStack.hasOwnProperty(series) && self.pointStack[series].length > 0) {
                    points = self.pointStack[series];
                    self.pointStack[series] = [];
                }
...
Renderer.prototype.drawHistory = function (from, until, onHistoryLoadedCallback, series) {
    ...
    if(typeof self._rendererLoadHistoryInProgress == "undefined")
        self._rendererLoadHistoryInProgress = {};

    if(!self._rendererLoadHistoryInProgress.hasOwnProperty(series))
        self._rendererLoadHistoryInProgress[series] = true;

	self.chart.onHistoryLoading();

	if (until === -1) {
		self.dataSource.getServerTimeOffset(function (e) {
            until = new Date().valueOf() + e +  5000;
			from =
				until -
                self.chart.xAxis[0].options.timeSpan * self.chart.xAxis[0].options.timeSpanUnit * 1000 - 5000;
	...
	this.dataSource.loadPoints(dataSourceOptions, function (dataSourceErrors, data, more) {
		...
		for (var dataKey in data) {
            ...
            for (var da = 0; da < lastIndex + 1; da++) {
                if(dataArray[da][0] != undefined && dataArray[da][1] != undefined) {
                    series.addPoint(dataArray[da], da == lastIndex);
                }
            }
        }
        ...
        if (series) {
            try {
                /* store current live points as gap */
                if (series.points) {
                ...
                /* concat history with live points */
                dataArray = dataArray.concat(gapData);

                /* re-create chart with combined histroy and live data */
                if (dataKey.indexOf("--high-value") == -1 && gapData.length > 0) {
                    series.setData([], false);
		...
		setTimeout(function (e) {
			self.updateNonStop();
            self._rendererLoadHistoryInProgress[series] = false;
