...
/*
  ...
  Also includes the modules Scope SDK (v1.0.3), Scope Proxy (v1.0.3)
*/
(function () {
  'use strict';
  ...
  function use(host) {
    ...
    webMI.data.call = function call(originalPath, originalOptions, callback) {
      ...
      const sameProtocol = protocol === "//" || protocol.startsWith(location.protocol);
      const sameOrigin = sameProtocol && host === location.hostname && port === location.port;
      if (m !== null && !sameOrigin) {
  ...
  ScopeDatasource.prototype._handleSubscriptionResults = function (
    ...
    if (!channelsMap || !results || !("channels" in results)) {
      // Ignore old subscription results or invalid results
      return {};
    }
