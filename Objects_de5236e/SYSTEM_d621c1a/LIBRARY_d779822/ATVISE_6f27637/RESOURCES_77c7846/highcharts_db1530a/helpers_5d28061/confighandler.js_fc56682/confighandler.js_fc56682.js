...
ConfigHandler.prototype.getConfigList = function (configNode, configMethode, configName, configCallback, sort) {
	...
	if (configMethode === "localStorage") {
        ...
        const index = Object.keys(stored ? saveJSONparse(stored) : {});
		...
		function grant() {
			webMI.data.read(configNode, function (e) {
				...
				var index = [];
                if (e.value) index = saveJSONparse(e.value);
		...
		function grant() {
			webMI.data.read(configNode, function (e) {
                ...
                const index = saveJSONparse(e.value);
		...
		function grant() {
			...
			that.file.read(filename, function (e) {
                const index = e.result != "" ? saveJSONparse(e.result) : {};
...
ConfigHandler.prototype.readConfig = function (configNode, configMethode, configName, userConfigCallback, merge) {
	...
	if (configMethode === "localStorage") {
        const stored = saveJSONparse(window.localStorage.getItem(configNode));

		var config;
		if (configName) {
			config = stored ? stored[configName] : null;
		...
		function grant() {
			webMI.data.read(configNode, function (e) {
                ...
                if (e.value) stored = saveJSONparse(e.value);
		...
		function grant() {
			var filename = "hcconfigs/" + configNode + ".json";
			that.file.read(filename, function (e) {
                const stored = e.result != "" ? saveJSONparse(e.result) : {};
...
ConfigHandler.prototype.writeConfig = function (chart, configNode, configName, configCallback, remove, importedConfig) {
	...
	if (configMethode === "localStorage") {
		var stored = window.localStorage.getItem(configNode);
        stored = stored ? saveJSONparse(stored) : {};
...
ConfigHandler.prototype.configurationChange = function (chart, configName, chartConfig, options) {
	...
	for (var i = 0; i < chartConfig.xAxis.length; i++) {
		/* fix hc axis y parameter */
		if(chartConfig.xAxis[i].hasOwnProperty("labels"))
			chartConfig.xAxis[i].labels.y = 0;
	...
	for (var i = 0; i < chartConfig.yAxis.length; i++) {
		/* fix hc axis y parameter */
		if(chartConfig.yAxis[i].hasOwnProperty("labels"))
			chartConfig.yAxis[i].labels.y = 0;
...
ConfigHandler.prototype.configurationFix = new (function () {
	...
	this.axisTicks = function (config, options) {
        for (var axis in config.xAxis) {
            __setProperty(config.xAxis[axis], "startOnTick", true);
            __setProperty(config.xAxis[axis], "endOnTick", true);
            __setProperty(config.xAxis[axis], "tickInterval", null);
            __setProperty(config.xAxis[axis], "minorTickInterval", null);
        }

		for (var axis in config.yAxis) {
            __setProperty(config.yAxis[axis], "startOnTick", true);
            __setProperty(config.yAxis[axis], "endOnTick", true);
            __setProperty(config.yAxis[axis], "tickInterval", null);
            __setProperty(config.yAxis[axis], "minorTickInterval", null);
		}
	...
	this.axisTitles = function (config, options) {
        for (var axis in config.xAxis) {
            __setProperty(config.xAxis[axis], "title", {});
            __setProperty(config.xAxis[axis].title, "align", "middle");
            __setProperty(config.xAxis[axis].title, "rotation", 0);
            __setProperty(config.xAxis[axis].title, "useHTML", false);

            __setProperty(config.xAxis[axis].title, "x", 0);
            __setProperty(config.xAxis[axis].title, "y", 0);

            __setProperty(config.xAxis[axis].title, "style", {});
            __setProperty(config.xAxis[axis].title.style, "color", "#666666");
            __setProperty(config.xAxis[axis].title, "text", translations["X-Axis"] + " " + axis);
        }

        for (var axis in config.yAxis) {
            __setProperty(config.yAxis[axis], "title", {});
            __setProperty(config.yAxis[axis].title, "align", "middle");
            __setProperty(config.yAxis[axis].title, "rotation", 270);
            __setProperty(config.yAxis[axis].title, "useHTML", false);

            __setProperty(config.yAxis[axis].title, "x", 0);
            __setProperty(config.yAxis[axis].title, "y", 0);

            __setProperty(config.yAxis[axis].title, "style", {});
            __setProperty(config.yAxis[axis].title.style, "color", "#666666");
            __setProperty(config.yAxis[axis].title, "text", translations["Y-Axis"] + " " + axis);
		}
    ...
    /**
     * set unset configuration parameter
     * @param obj
     * @param property
     * @param defaultvalue
     * @private
     */
    function __setProperty(obj, property, defaultvalue) {
