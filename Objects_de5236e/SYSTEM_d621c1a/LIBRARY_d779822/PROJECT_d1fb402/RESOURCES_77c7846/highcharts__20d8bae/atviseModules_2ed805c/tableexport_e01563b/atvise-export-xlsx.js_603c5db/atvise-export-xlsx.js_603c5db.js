/* modified by Bachmann Visutec GmbH */
(function (H) {
	...
	// Fix for ghost button > AT-D-14147
	var contextButtonEnabled = H.getOptions().exporting.buttons.contextButton.enabled;
	var buttonOptionsEnabled = H.getOptions().navigation.buttonOptions.enabled;

	if (contextButtonEnabled == undefined || contextButtonEnabled == "undefined") {
		H.getOptions().exporting.buttons.contextButton.enabled = true;
	}

	if (buttonOptionsEnabled == undefined || buttonOptionsEnabled == "undefined") {
		H.getOptions().navigation.buttonOptions.enabled = false;
	}
	...
	H.Chart.prototype.downloadXLSX = function () {
		...
		if (false && chart.axes[0].isDatetimeAxis) { /* modified by Bachmann Visutec GmbH for string output */
		...
		if (xlsxOptions.worksheet.autoFitColumns === true) {
			...
			for (var col = 0; col < worksheet.columns.length; col++) {
				...
				var longest = columnFormattedValues.reduce(function (a, b) {
