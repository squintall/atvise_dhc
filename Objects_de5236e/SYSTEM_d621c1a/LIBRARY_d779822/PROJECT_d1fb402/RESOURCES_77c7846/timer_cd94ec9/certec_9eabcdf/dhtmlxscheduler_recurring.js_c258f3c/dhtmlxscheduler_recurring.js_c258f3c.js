...
2) modified variable - scheduler.locale.labels using
...
scheduler.form_blocks.recurring = {
	...
	button_click: function (a, b, c, d) {
		!d.open && !d.blocked ? (d.style.height = "105px", b.style.backgroundPosition = "-5px 0px", b.nextSibling.innerHTML = scheduler.locale.labels.button_recurring_open) : (d.style.height = "0px", b.style.backgroundPosition = "-5px 20px", b.nextSibling.innerHTML = scheduler.locale.labels.button_recurring);
		d.open = !d.open;
...
scheduler.attachEvent("onEventChanged", function (a) {
	...
	var b = this.getEvent(a);
	if (b.event_pid != "") {
		var b2 = this.getEvent(b.event_pid);
		this._not_render = !0;

		var f = this._get_rec_markers(b.event_pid);
		for (var e in f) f.hasOwnProperty(e) && (delete this._rec_markers[f[e].id], this.deleteEvent(f[e].id, !0));
		delete this._rec_markers_pull[b.event_pid];
		var need_end_mod = false;
		var type = b2.rec_type.split("#")[1];
		var patt,e;
		if (b.start_date < b2.start_date){
			patt = b2.rec_pattern.split("_");
			e = patt[0] == "week" && patt[4] && patt[4].toString().indexOf(scheduler.config.start_on_monday ? 1 : 0) == -1 ? 1 : 0;
			b2.start_date = b.start_date;
			need_end_mod = true;
		} else if (b.end_date > b2.end_date && type != "" && type != "no"){
			patt = b2.rec_pattern.split("_");
			e = patt[0] == "week" && patt[4] && patt[4].toString().indexOf(scheduler.config.start_on_monday ? 1 : 0) == -1 ? 1 : 0;
			var computed_end_date = scheduler.date.add(scheduler.date.add(scheduler.date.add(b2.start_date, parseInt(type) + e, patt.join("_")),b2.event_length/60,"minute"),-1,"day");
			var slice = (b.end_date.getTime() - computed_end_date.getTime())/(1000*60);
			b2.start_date = scheduler.date.add(b2.start_date,slice,"minute");
			need_end_mod = true;
		} else if (b.end_date > b2.end_date && type == ""){
			need_end_mod = true;
		} else {
			b2.start_date = new Date(b2.start_date.getFullYear(),b2.start_date.getMonth(),b2.start_date.getDate(),b.start_date.getHours(),b.start_date.getMinutes(),b.start_date.getSeconds());
		}
		b2.event_length = (new Date(b.end_date).getTime()-new Date(b.start_date).getTime())/1000;
		b2.text = b.text;
		var type;
		if ( need_end_mod && type != "" && type != "no"){
			b2.end_date = scheduler.date.add(new Date(b2.start_date), parseInt(type) + e, patt.join("_"));
			b2.end_date = new Date(b2.end_date.getFullYear(),b2.end_date.getMonth(),b2.end_date.getDate());
		} else if (need_end_mod && type == ""){
			b2.end_date = scheduler.date.add(new Date(b.end_date),1,"day");
			b2.end_date = new Date(b2.end_date.getFullYear(),b2.end_date.getMonth(),b2.end_date.getDate());
		}
		this._not_render = !1;
	} else {
